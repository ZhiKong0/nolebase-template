...
volatile float imu_dbg_gyro_z_raw = 0.0f;
volatile float imu_dbg_gyro_z_offset = 0.0f;
volatile float imu_dbg_gyro_z_comp = 0.0f;
...
static float gyro_z_offset_accum = 0.0f;
static int gyro_z_offset_count = 0;
static int gyro_z_offset_ready = 0;
...
void IMU_getValues(float * values) {  
	...
	if (!gyro_z_offset_ready)
	{
		gyro_z_offset_accum += gyroval.z;
		gyro_z_offset_count++;
		if (gyro_z_offset_count >= 200)
		{
			gyro_offset[2] = gyro_z_offset_accum / (float)gyro_z_offset_count;
			gyro_z_offset_ready = 1;
			ezInt = 0;
		}
	}
	...
	imu_dbg_gyro_z_raw = gyroval.z;
	imu_dbg_gyro_z_offset = gyro_offset[2];
	imu_dbg_gyro_z_comp = gyroval.z - gyro_offset[2];
    values[0] =  accval.x;
    values[1] =  accval.y;
    values[2] =  accval.z;
