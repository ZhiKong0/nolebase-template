...
# 烧录后自动复位 MCU，无需手动断电上电
& $PYOCD reset --no-config -t $targetOk
Write-Host "MCU reset done."
```
...
- **无需手动断电上电**：`-M under-reset` 模式会在连接时自动 hold reset，烧录完成后再执行 `pyocd reset` 即可让 MCU 以新固件启动。只有在 `erase` 或 `load` 都失败时才需要手动断电上电
- **永远不要用 `pyocd flash` 直接烧录，应始终使用 `erase --chip` + `load` 两步流程**：`pyocd flash` 是一个简化命令，内部尝试按页擦写，但在 STM32F103 + DAPLink 组合下频繁在首页 `0x08000000` 触发 `page failure`（2026-04-10 再次复现）。根因是 Flash 控制器在未全片擦除时可能处于不一致状态（旧数据残留、写保护位未清除等）。**推荐做法**：始终先 `erase --chip`，再 `load`，成功率接近 100%。如果连 `erase` 都失败，先断电上电再试
