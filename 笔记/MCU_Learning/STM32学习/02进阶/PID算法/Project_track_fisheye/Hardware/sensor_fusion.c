...
static uint8_t  s_bnoEverReady = 0u;
...
static uint8_t BNO_ReceivePacketEx(uint8_t *channel, uint16_t *payloadLen, uint8_t forcePoll)
{
    ...
    /* Read only the remaining payload bytes. The 4-byte SHTP header was already
       consumed above; reading len+4 here shifts payload parsing and breaks
       control/report decoding during init. */
    if (!bno_read_block(raw, len)) { s_bnoLastRxFailCode = 4u; return 0u; }

    memset(s_bnoData, 0, sizeof(s_bnoData));
    for (i = 0; i < len; i++) s_bnoData[i] = raw[i];
...
static uint8_t bno_attempt_init_at_addr(uint8_t addr, uint8_t doReset, IMU_Data_t *tempData)
{
    bno_set_addr(addr);
    ...
    if (!bno_request_product_id()) {
        /* BNO08x on I2C is known to be unfriendly to generic zero-length
           address scans. Use an actual SHTP control transaction as the
           presence test and fail here only if the handshake itself fails. */
        bno_set_fail(BNO_FAIL_PRODUCT_ID);
        bno_set_addr(0u);
        return 0u;
    }
...
void BNO085_Init(void)
{
    ...
    s_bnoReady = 0u;
    s_bnoEverReady = 0u;
    s_bnoInitStage = BNO_INIT_IDLE;
    s_bnoInitFailCode = BNO_FAIL_NONE;
    s_bnoInitBusy = 0u;
    s_bnoReadFailStreak = 0u;
    s_bnoNoNewDataCount = 0u;
    s_bnoRxAttemptCount = 0u;
    s_bnoRxPacketCount = 0u;
    s_bnoLastRxFailCode = 0u;
    s_bnoLastChannel = 0u;
    s_bnoLastReportId = 0u;
    s_bnoLastPayloadLen = 0u;
    return;
#else
    ...
    for (i = 0; i < (uint8_t)(sizeof(addrList) / sizeof(addrList[0])); i++) {
        if (bno_attempt_init_at_addr(addrList[i], 0u, &tempData)) break;
    }

    if (s_bnoWho != 0x85u) {
        for (i = 0; i < (uint8_t)(sizeof(addrList) / sizeof(addrList[0])); i++) {
            if (bno_attempt_init_at_addr(addrList[i], 1u, &tempData)) break;
        }
    }
...
static uint8_t bno_read_all_core(IMU_Data_t *data, uint8_t forcePoll)
{
    ...
    if (!s_bnoReady) {
        if (!s_bnoInitBusy && s_bnoEverReady) {
...
{
#if !IMU_ENABLE
    return BNO_INIT_IDLE;
#else
    return s_bnoInitStage;
#endif
}
...
{
#if !IMU_ENABLE
    return BNO_FAIL_NONE;
#else
    return s_bnoInitFailCode;
#endif
}
...
static void line_mux_delay(void)
{
    ...
    for (guard = 0u; guard < LINE_MUX_SETTLE_CYCLES; ++guard) {
        __NOP();
    }
}
...
{
    GPIO_WriteBit(LINE_MUX_S0_PORT, LINE_MUX_S0_PIN,
                  (BitAction)((channel & 0x01u) ? Bit_SET : Bit_RESET));
    GPIO_WriteBit(LINE_MUX_S1_PORT, LINE_MUX_S1_PIN,
                  (BitAction)((channel & 0x02u) ? Bit_SET : Bit_RESET));
    GPIO_WriteBit(LINE_MUX_S2_PORT, LINE_MUX_S2_PIN,
                  (BitAction)((channel & 0x04u) ? Bit_SET : Bit_RESET));
    line_mux_delay();
}
...
void LineSensor_Init(void)
{
    ...
    g.GPIO_Mode = GPIO_Mode_Out_PP;
    g.GPIO_Speed = GPIO_Speed_50MHz;
    g.GPIO_Pin = LINE_MUX_S0_PIN | LINE_MUX_S1_PIN | LINE_MUX_S2_PIN;
    GPIO_Init(GPIOA, &g);

    GPIO_ResetBits(LINE_MUX_S0_PORT, LINE_MUX_S0_PIN);
    GPIO_ResetBits(LINE_MUX_S1_PORT, LINE_MUX_S1_PIN);
    GPIO_ResetBits(LINE_MUX_S2_PORT, LINE_MUX_S2_PIN);
...
{
    uint8_t level = (GPIO_ReadInputDataBit(LINE_MUX_Z_PORT, LINE_MUX_Z_PIN) == Bit_SET) ? 1u : 0u;
#if LINE_ACTIVE_LOW
    return level ? 0u : 1u;
#else
    return level;
