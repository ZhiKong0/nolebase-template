...
MODE_COMMAND_SPECS = {
    MODE_STRAIGHT: {
        ...
        "default_target_speed": 10.0,
    },
    MODE_TRACK: {
        "config_key": "track",
        "pid_entries": (("speed_pid", "SPEED"), ("line_pid", "LINE")),
        "default_target_speed": 20.0,
    },
}
...
def normalize_track_line_pid(line_pid: Any) -> Optional[tuple[float, float]]:
    """Current firmware exposes only LKP/LKD at runtime.

    Accept:
      [kp, kd]          -> current format
      [kp, legacy_ki, kd] -> legacy config, silently drop KI
    """
    if not isinstance(line_pid, list):
        return None
    if len(line_pid) == 2:
        return float(line_pid[0]), float(line_pid[1])
    if len(line_pid) == 3:
        return float(line_pid[0]), float(line_pid[2])
    return None
...
def build_setup_commands(cfg: dict[str, Any]) -> list[str]:
    ...
    if protocol == "TEXT":
        ...
        if "feedforward_gain" in mode_cfg:
            commands.append(f"#SFF={float(mode_cfg['feedforward_gain'])}!")
        if mode == MODE_STRAIGHT:
            ...
            line_pid = normalize_track_line_pid(mode_cfg.get("line_pid"))
            if line_pid is not None:
                commands.append(f"#LKP={line_pid[0]}!")
                commands.append(f"#LKD={line_pid[1]}!")
