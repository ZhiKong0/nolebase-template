...
static uint32_t experiment_display_id(void)
{
    return experiment_host_sync_active() ? g_experimentId : 0u;
}
...
static void transition_start(ExperimentTrigger_t trigger)
{
    ...
    BspOled_ShowExperimentId(experiment_display_id());
    g_displayDirty = 1;
}
...
static void handle_command(const char *cmd)
{
    ...
    if (strncmp(cmd, "#SPD=", 5) == 0)
    {
        if (cmd_parse_float(cmd + 5, &fval))
        {
            if (g_mode == MODE_TRACK)
                LineTrack_SetBasePwm((int16_t)fval);
            else
    ...
    if (strcmp(cmd, "#STAT!") == 0)
    {
        BspUart_SendStat(g_sysState, g_mode,
                         ...
                         (g_mode == MODE_TRACK) ? (float)LineTrack_GetBasePwm() : g_pid.targetSpeed);
        return;
    }
    if (strcmp(cmd, "#CAL!") == 0)
    {
