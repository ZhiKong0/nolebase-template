...
#define EXP_HOST_SYNC_TIMEOUT_MS 4000 /* py 端实验编号接管租期(ms), 超时后板子不再自动递增编号 */
...
/* ========== 循迹模式: 八路输入 + 五路转角逻辑 ========== */
/* 说明:
 * 1. 正常循迹直接使用 8 路传感器的加权位置做控制输入。
 * 2. 转角/全灭后恢复仍按五路参考代码：
 *    8 路先映射成 L1/L0/M/R0/R1，再根据 last_data 决定左右转。
 * 3. #SPD= 在循迹模式下表示基础 PWM，而不是编码器目标速度。 */
#define PID_TRACK_SPEED_TARGET   280.0f  /* 仅保留给原有状态显示接口，实际循迹按固定 base PWM 跑 */
#define PID_TRACK_SPEED_KP       1.50f   /* 兼容旧接口，八路循迹主链不使用该速度环 */
...
#define TRACK_TURN_RIGHT_L_PWM   220
#define TRACK_TURN_RIGHT_R_PWM   180
#define TRACK_CORNER_STOP_DELAY_MS 5

#define TRACK_OVERRUN_LIMIT      50      /*  50 次全灭停车判定 */
#define TRACK_DEFAULT_CROSSINGS  4
#define TRACK_CROSS_FILTER       50
