...
- **Status:** complete
- Actions taken:
  - 保留 `USART1(PB6/PB7 remap)` 作为 `DAPlink / 命令串口`
  - 把 `BNO085_RST` 从 `PB14` 改到 `PB11`
  - 在 [`Hardware/config.h`](/F:/Documents/GitHub/nolebase-template/笔记/MCU_Learning/STM32学习/02进阶/PID算法/Project_Refactor_track-421%20-%20副本%20(3)/Hardware/config.h) 中补充 `PB14/PB15` 无线串口预留宏
...
- 代码主线已经回到真实硬件边界
- 文档已同步修正
- 下一步如果要上板，需要先确认 `BNO085_RST -> PB11` 是否已实际改线
