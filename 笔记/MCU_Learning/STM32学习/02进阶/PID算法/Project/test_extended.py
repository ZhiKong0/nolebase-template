"""
Extended test - 20 seconds to give more time for button press
"""
...
def analyze_car_behavior(data_list):
    ...
    if left_pwm_values and right_pwm_values:
        ...
        if abs(pwm_diff) > 10:
            if pwm_diff > 0:
                analysis.append(f"\n[WARNING] Car drifting RIGHT (left PWM > right by {pwm_diff:.1f})")
            else:
                analysis.append(f"\n[WARNING] Car drifting LEFT (right PWM > left by {-pwm_diff:.1f})")
        else:
            analysis.append(f"\n[OK] Going relatively straight (PWM diff = {pwm_diff:.1f})")
        ...
        if len(left_pwm_values) > 1:
            ...
            if left_variance > 100 or right_variance > 100:
                analysis.append(f"\n[WARNING] JERKY motion (high PWM variance)")
            else:
    ...
    if left_enc_values and right_enc_values:
        ...
        analysis.append(f"\nEncoder Status:")
        analysis.append(f"  Left:  {'WORKING' if left_enc_changing else 'STUCK'} (range: {min(left_enc_values)} to {max(left_enc_values)})")
        analysis.append(f"  Right: {'WORKING' if right_enc_changing else 'STUCK'} (range: {min(right_enc_values)} to {max(right_enc_values)})")
...
print("="*60)
print("Extended Test - 20 seconds")
print("="*60)
print(f"Log file: {log_file}")
print("="*60)
print("\n[INSTRUCTION] Press PB5 button ANYTIME in the next 20 seconds!")
print("[INSTRUCTION] The car will start when you press the button\n")
