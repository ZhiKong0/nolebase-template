"""
Comprehensive test - send #RUN! multiple times and analyze car behavior
"""
...
def analyze_car_behavior(data_list):
    ...
    if run_count == 0:
        ...
        analysis.append("  1. #RUN command not working")
        analysis.append("  2. Button not pressed")
        analysis.append("  3. Motor enable failed")
        return "\n".join(analysis)
    ...
    if left_pwm_values and right_pwm_values:
        ...
        if len(left_pwm_values) > 1:
            ...
            if left_variance > 100 or right_variance > 100:
                ...
                analysis.append("  1. PID parameters not tuned")
                analysis.append("  2. Encoder noise")
                analysis.append("  3. Mechanical issues")
            else:
    ...
    if left_enc_values and right_enc_values:
        ...
        analysis.append(f"\nEncoder Status:")
        analysis.append(f"  Left encoder:  {'WORKING' if left_enc_changing else 'STUCK'} (values: {min(left_enc_values)} to {max(left_enc_values)})")
        analysis.append(f"  Right encoder: {'WORKING' if right_enc_changing else 'STUCK'} (values: {min(right_enc_values)} to {max(right_enc_values)})")

        if not left_enc_changing or not right_enc_changing:
            analysis.append(f"\n[WARNING] Encoder not changing - wheels may not be moving!")

    # Analyze yaw (heading)
    yaw_values = [d.get('y', 0) for d in data_list if d.get('run', 0) == 1]
    yaw_err_values = [d.get('e', 0) for d in data_list if d.get('run', 0) == 1]

    if yaw_values:
        yaw_range = max(yaw_values) - min(yaw_values)
        analysis.append(f"\nHeading Analysis:")
        analysis.append(f"  Yaw range: {yaw_range} degrees")

        if yaw_range > 30:
            analysis.append(f"\n[WARNING] Large heading change - car is turning significantly")
        elif yaw_range > 10:
            analysis.append(f"\n[INFO] Moderate heading change - some drift")
        else:
            analysis.append(f"\n[OK] Heading stable")
...
def run_comprehensive_test(duration=10):
    """Comprehensive test with behavior analysis"""
    ...
    print(f"\n{'='*60}")
    print(f"Comprehensive Car Test with Behavior Analysis")
    print(f"{'='*60}")
    print(f"Test duration: {duration} seconds")
    print(f"Log file: {log_file}")
    print(f"{'='*60}\n")

    # Send #RUN! command multiple times to ensure it's received
    print("[INFO] Sending #RUN! command (3 attempts)...")
    for i in range(3):
        ser.write(b"#RUN!")
        time.sleep(0.2)
        if ser.in_waiting > 0:
            response = ser.read(ser.in_waiting).decode('ascii', errors='ignore')
            if 'OK RUN' in response:
                print(f"[OK] Car started successfully!")
                break
            elif 'run=1' in response:
                print(f"[OK] Car is running (detected from data stream)")
                break
    else:
        print("[WARNING] No 'OK RUN' response received, but continuing test...")
...
if __name__ == '__main__':
    print("=" * 60)
    print("STM32 Car Comprehensive Test with Behavior Analysis")
    print("=" * 60)
    print()

    run_comprehensive_test(duration=10)
