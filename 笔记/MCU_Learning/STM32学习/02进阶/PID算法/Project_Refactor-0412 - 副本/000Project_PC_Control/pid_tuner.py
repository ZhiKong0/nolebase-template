...
class HBRecord:
    ...
    hi: float = 0.0     # heading PID integral (accumulated correction)
...
def run_capture(port_name: str, baudrate: int, duration_s: float,
    ...
    with serial.Serial(port_name, baudrate=baudrate, timeout=0.1) as port:
        ...
        port.write(b"#STOP!")
        time.sleep(0.5)
        flush_serial(port, wait_ms=500)

        # 1) Set mode (retry up to 3 times)
        resp = []
        for attempt in range(3):
            resp = send_cmd(port, "#MODE=STRAIGHT!", timeout_s=2.0)
            print(f"  MODE resp (attempt {attempt+1}): {resp}")
            if any("OK" in l for l in resp):
                break
            time.sleep(0.5 * (attempt + 1))
            flush_serial(port, wait_ms=200)
        ...
        for key, prefix in param_cmds.items():
            if key in pid_params:
                val = pid_params[key]
                # MCU firmware uses sscanf("%f") — send 5 decimal places for 0.001 resolution
                cmd = f"{prefix}{val:.5f}!"
...
def reconstruct_trajectory(records: list[HBRecord]) -> list[TrajectoryPoint]:
    ...
    IMPORTANT: el/er are accumulated encoder counts since the LAST
    Encoder_Update() call (= one control tick), NOT counts-per-10ms.
    They already represent the total distance for each sample interval.
    Do NOT multiply by dt again.

    y > 0 = LEFT drift (CCW-positive math convention).
    """
    if not records:
        return []
    ...
    prev_t = t0
    for r in records:
        # Deduplicate: skip records with same timestamp as previous
        if r.t_ms == prev_t and pts:
            continue
        # el, er are already the total encoder counts for this interval
        distance = (r.el + r.er) / 2.0
        # Trapezoidal: use average of previous and current heading
        # MCU yaw is CW-positive (positive=right); negate for CCW-positive math
...
def estimate_drift_multi_method(records: list[HBRecord], startup_ms: int = 2000) -> dict:
    """Estimate drift direction using multiple independent methods.

    Returns dict with per-method results and consensus.

    Sign conventions (verified: config.h TIM2=left, TIM3=right):
      el = LEFT physical wheel, er = RIGHT physical wheel
      MCU yaw CW+: yaw>0 = turning RIGHT
      Car curves RIGHT → left wheel outside → el>er → Σ(er-el)<0
      Car curves LEFT  → right wheel outside → er>el → Σ(er-el)>0
      hd>0 → OL>OR → left motor faster → car pushed RIGHT → correcting LEFT drift
      hi>0 → accumulated correction for leftward heading → net LEFT lateral drift

    NOTE: encoder diff has systematic hardware bias (el consistently > er).
    This method uses bias-corrected encoder diff and gives odometry highest weight.
    """
    t0 = records[0].t_ms if records else 0
    ss = [r for r in records if (r.t_ms - t0) > startup_ms and r.el > 0]
    if len(ss) < 10:
        return {'consensus': 'unknown', 'confidence': 0, 'methods': {}}

    methods = {}

    # --- Encoder asymmetry detection ---
    # Compute mean(el) vs mean(er) to detect systematic hardware bias
    mean_el = statistics.mean([r.el for r in ss])
    mean_er = statistics.mean([r.er for r in ss])
    enc_ratio = mean_el / max(mean_er, 0.1)
    enc_bias_per_sample = mean_el - mean_er  # positive = el reads higher

    # --- Method 1: Encoder differential (BIAS-CORRECTED) ---
    # Subtract the systematic bias so we only see real curvature
    enc_diff_raw = sum(r.er - r.el for r in ss)
    enc_diff_corrected = enc_diff_raw + enc_bias_per_sample * len(ss)  # remove bias
    enc_corr_per_sample = enc_diff_corrected / len(ss)
    # Higher threshold after bias correction (residual noise)
    if abs(enc_corr_per_sample) > 0.5:
        methods['enc_diff'] = {'dir': '左' if enc_diff_corrected > 0 else '右',
                               'value': round(enc_corr_per_sample, 3),
                               'total': round(enc_diff_corrected),
                               'raw_total': enc_diff_raw,
                               'bias': round(enc_bias_per_sample, 2),
                               'desc': f'Σ(er-el)校正={enc_diff_corrected:+.0f} '
                                       f'(原始{enc_diff_raw:+.0f}, 偏差{enc_bias_per_sample:+.1f}/样)'}
    else:
        methods['enc_diff'] = {'dir': '中', 'value': round(enc_corr_per_sample, 3),
                               'total': round(enc_diff_corrected),
                               'raw_total': enc_diff_raw,
                               'bias': round(enc_bias_per_sample, 2),
                               'desc': f'Σ(er-el)校正={enc_diff_corrected:+.0f} '
                                       f'(原始{enc_diff_raw:+.0f}, 偏差{enc_bias_per_sample:+.1f}/样, 噪声内)'}

    # --- Method 2: PWM differential ---
    # OL>OR → left motor faster → car pushed RIGHT → correcting LEFT drift
    pwm_diffs = [r.ol - r.or_ for r in ss]
    pwm_mean = statistics.mean(pwm_diffs)
    if abs(pwm_mean) > 0.5:
        methods['pwm_diff'] = {'dir': '左' if pwm_mean > 0 else '右',
                               'value': round(pwm_mean, 2),
                               'desc': f'mean(OL-OR)={pwm_mean:+.2f} → PID修正{"→右" if pwm_mean > 0 else "→左"} → 自然{"左" if pwm_mean > 0 else "右"}偏'}
    else:
        methods['pwm_diff'] = {'dir': '中', 'value': round(pwm_mean, 2),
                               'desc': f'mean(OL-OR)={pwm_mean:+.2f} (噪声内)'}

    # --- Method 3: Heading integral (hi) ---
    # hi>0 → accumulated rightward correction → natural LEFT drift
    hi_final = ss[-1].hi
    hi_start = ss[0].hi
    hi_delta = hi_final - hi_start
    if abs(hi_final) > 0.1:
        methods['hi'] = {'dir': '左' if hi_final > 0 else '右',
                         'value': round(hi_final, 3),
                         'delta': round(hi_delta, 3),
                         'desc': f'hi_final={hi_final:+.3f} delta={hi_delta:+.3f} → 自然{"左" if hi_final > 0 else "右"}偏'}
    else:
        methods['hi'] = {'dir': '中', 'value': round(hi_final, 3),
                         'delta': round(hi_delta, 3),
                         'desc': f'hi={hi_final:+.3f} (噪声内)'}

    # --- Method 4: Yaw linear trend ---
    # yaw trending positive (CW) → car turning RIGHT
    ts = [(r.t_ms - t0) / 1000.0 for r in ss]
    yaws = [r.yaw for r in ss]
    yaw_slope = _linfit(ts, yaws)
    if abs(yaw_slope) > 0.005:
        methods['yaw_trend'] = {'dir': '右' if yaw_slope > 0 else '左',
                                'value': round(yaw_slope, 4),
                                'desc': f'yaw斜率={yaw_slope:+.4f}°/s → {"右转" if yaw_slope > 0 else "左转"}趋势'}
    else:
        methods['yaw_trend'] = {'dir': '中', 'value': round(yaw_slope, 4),
                                'desc': f'yaw斜率={yaw_slope:+.4f}°/s (噪声内)'}

    # --- Method 5: Corrected odometry ---
    # Use fixed trajectory reconstruction (no dt inflation)
    x, y = 0.0, 0.0
    prev_yaw = ss[0].yaw
    for r in ss:
        dist = (r.el + r.er) / 2.0
        mid_rad = -math.radians((prev_yaw + r.yaw) / 2.0)
        x += dist * math.cos(mid_rad)
        y += dist * math.sin(mid_rad)
        prev_yaw = r.yaw
    lat_mm = y * ENC_TO_MM
    if abs(lat_mm) > 2.0:
        methods['odometry'] = {'dir': '左' if lat_mm > 0 else '右',
                               'value': round(lat_mm, 1),
                               'desc': f'修正里程计: lat={lat_mm:+.1f}mm → {"左" if lat_mm > 0 else "右"}偏'}
    else:
        methods['odometry'] = {'dir': '中', 'value': round(lat_mm, 1),
                               'desc': f'修正里程计: lat={lat_mm:+.1f}mm (噪声内)'}

    # --- Consensus voting ---
    # Only 左/右 compete; 中(undecided) does NOT block consensus
    # Odometry = highest weight (combines heading+distance, robust to encoder bias)
    # Encoder diff = reduced weight (susceptible to systematic hardware bias)
    votes = {'左': 0, '右': 0, '中': 0}
    weights = {'enc_diff': 2, 'pwm_diff': 2, 'hi': 2, 'yaw_trend': 1, 'odometry': 4}
    for name, m in methods.items():
        w = weights.get(name, 1)
        votes[m['dir']] += w
    decided_w = votes['左'] + votes['右']

    # Magnitude qualifier: very small drift = uncertain
    odo_val = abs(methods.get('odometry', {}).get('value', 0))

    if decided_w == 0:
        consensus = '基本居中'
        confidence = 100
    elif votes['左'] > votes['右']:
        consensus = '微左偏' if odo_val < 10 else '左偏'
        confidence = round(votes['左'] / max(decided_w, 1) * 100)
    elif votes['右'] > votes['左']:
        consensus = '微右偏' if odo_val < 10 else '右偏'
        confidence = round(votes['右'] / max(decided_w, 1) * 100)
    else:
        consensus = '方向不确定'
        confidence = 50
    ...
    return {
        ...
        'votes': dict(votes),
        'methods': methods,
        'enc_asymmetry': round(enc_ratio, 4),
        'enc_bias_per_sample': round(enc_bias_per_sample, 2),
    }
...
class StraightLineReport:
    ...
    # --- Raw Speed Continuity (原始数据, 无平滑) ---
    raw_ol_std: float = 0.0              # OL标准差 (越小越稳)
    raw_ol_slew_mean: float = 0.0        # OL逐采样变化均值
    raw_ol_slew_max: int = 0             # OL最大突变
    raw_ol_slew_gt20_pct: float = 0.0    # OL突变>20的比例
    raw_pc_stall_pct: float = 0.0        # pc<=1的比例 (电机停/最低速)
    raw_pc_std: float = 0.0              # pc原始标准差
    raw_pc_range: int = 0                # pc振幅 (max-min)
    raw_el_cv: float = 0.0               # el变异系数 (速度一致性)

    # --- Multi-method Drift Analysis (多方法漂移分析) ---
    drift_consensus: str = ""             # 左偏/右偏/基本居中
    drift_confidence: int = 0             # 共识置信度 %
    drift_methods: dict = field(default_factory=dict)  # 各方法详情
    ...
    fr_cruise_lat_drift_dir: str = ""       # "左偏(+)" / "右偏(-)" / "稳定"  (y>0=左偏: yaw>0→sin>0→y>0)
...
def analyze_straight(records: list[HBRecord], params: dict[str, Any],
    ...
    if abs(yaw_mean) > 1.0 and abs(rpt.heading_mean_hd) > 0.1:
        # MCU yaw is CW-positive: yaw_mean<0 = left drift → hd should be positive (left faster → correct right)
        # yaw_mean>0 = right drift → hd should be negative (right faster → correct left)
    ...
    # ============ RAW SPEED CONTINUITY (原始数据, 无平滑) ============
    # Steady-state only (skip startup transient)
    ss_ols = [r.ol for r in run_recs if (r.t_ms - t0) > startup_ms]
    ss_pcs = [r.pc for r in run_recs if (r.t_ms - t0) > startup_ms]
    ss_els = [r.el for r in run_recs if (r.t_ms - t0) > startup_ms]
    if len(ss_ols) > 2:
        rpt.raw_ol_std = round(statistics.stdev(ss_ols), 2)
        ol_slews_ss = [abs(ss_ols[i] - ss_ols[i-1]) for i in range(1, len(ss_ols))]
        rpt.raw_ol_slew_mean = round(statistics.mean(ol_slews_ss), 2)
        rpt.raw_ol_slew_max = max(ol_slews_ss)
        rpt.raw_ol_slew_gt20_pct = round(sum(1 for s in ol_slews_ss if s > 20) / len(ol_slews_ss) * 100, 1)
    if len(ss_pcs) > 2:
        rpt.raw_pc_stall_pct = round(sum(1 for p in ss_pcs if p <= 1) / len(ss_pcs) * 100, 1)
        rpt.raw_pc_std = round(statistics.stdev(ss_pcs), 2)
        rpt.raw_pc_range = max(ss_pcs) - min(ss_pcs)
    if len(ss_els) > 2:
        el_mean = statistics.mean(ss_els)
        el_std = statistics.stdev(ss_els)
        rpt.raw_el_cv = round(el_std / max(el_mean, 1) * 100, 1)

    # ============ MULTI-METHOD DRIFT ANALYSIS ============
    drift_result = estimate_drift_multi_method(run_recs, startup_ms)
    rpt.drift_consensus = drift_result.get('consensus', '')
    rpt.drift_confidence = drift_result.get('confidence', 0)
    rpt.drift_methods = drift_result.get('methods', {})
    ...
    # ============ STRICT v10 SCORING (raw-data based, speed continuity) ============
    # Speed (25 pts): ALL based on RAW steady-state data, no smoothing/windowing
    # v10: OL stability is the ground truth — it's the actual motor PWM the user sees/feels
    sol_std_pen = min(rpt.raw_ol_std * 0.4, 8)           # OL波动: std=5→2pt, 20→8pt
    sol_slew_pen = min(rpt.raw_ol_slew_gt20_pct * 0.5, 5)  # OL突变>20占比: 2%→1pt, 10%→5pt
    spc_stall_pen = min(rpt.raw_pc_stall_pct * 0.8, 6)   # pc<=1(电机最低速): 2%→1.6pt, 7%→5.6pt
    spc_range_pen = min(max(rpt.raw_pc_range - 30, 0) * 0.04, 4)  # pc振幅>30: 80→2pt, 130→4pt
    sel_cv_pen = min(max(rpt.raw_el_cv - 20, 0) * 0.08, 4)  # el变异系数>20%: 40%→1.6pt, 70%→4pt
    rpt.score_speed = round(max(0, 25 - sol_std_pen - sol_slew_pen - spc_stall_pen - spc_range_pen - sel_cv_pen), 2)
    ...
    # ----- Speed Continuity (原始数据诊断) -----
    if rpt.raw_ol_std > 12:
        recs.append(f"🔴 速度时快时慢! OL_std={rpt.raw_ol_std:.1f} (目标<5)")
        recs.append(f"    pc振幅={rpt.raw_pc_range}, pc停顿={rpt.raw_pc_stall_pct:.1f}%, OL突变={rpt.raw_ol_slew_gt20_pct:.1f}%")
        recs.append("    建议: 调 SFF 使 ff≈DZ+7, 或降 SKP/SKI")
    elif rpt.raw_ol_std > 6:
        recs.append(f"⚠ 速度有波动 OL_std={rpt.raw_ol_std:.1f} (目标<5)")
        recs.append(f"    pc振幅={rpt.raw_pc_range}, pc停顿={rpt.raw_pc_stall_pct:.1f}%, OL突变={rpt.raw_ol_slew_gt20_pct:.1f}%")
    elif rpt.raw_ol_std > 0:
        recs.append(f"✅ 速度连贯 OL_std={rpt.raw_ol_std:.1f}, el_CV={rpt.raw_el_cv:.1f}%")
    if rpt.raw_pc_stall_pct > 5:
        recs.append(f"⚠ 电机频繁停顿 pc<=1占{rpt.raw_pc_stall_pct:.1f}%, 检查 SFF 和死区")

    # ----- Drift direction (multi-method consensus) -----
    if rpt.drift_consensus and rpt.drift_consensus not in ('基本居中', 'unknown', '方向不确定', ''):
        drift_icon = '←' if '左' in rpt.drift_consensus else '→'
        lat_val = rpt.drift_methods.get('odometry', {}).get('value', 0)
        enc_val = rpt.drift_methods.get('enc_diff', {}).get('value', 0)
        recs.append(f"  漂移: {drift_icon} {rpt.drift_consensus} "
                    f"(置信{rpt.drift_confidence}%, 里程计={lat_val:+.1f}mm, 编码器差={enc_val:+.2f}/样)")
        # Suggest HEADING_TRIM to compensate
        if abs(lat_val) > 5:
            # Estimate trim: 1° heading ≈ 17.5mm/m lateral at current speed
            fwd_mm = rpt.traj_total_distance * ENC_TO_MM
            fwd_m = max(fwd_mm / 1000.0, 0.01)
            drift_per_m = lat_val / fwd_m  # mm/m
            # sin(trim) ≈ drift_per_m/1000; trim ≈ drift_per_m/1000 * 180/π
            suggested_trim = -drift_per_m / 1000.0 * 57.3  # opposite direction
            recs.append(f"    建议 HEADING_TRIM ≈ {suggested_trim:+.2f}° 或增大 AKI 加速自纠")
    elif rpt.drift_consensus == '基本居中':
        recs.append("  漂移: · 基本居中")

    # ----- Heading PID -----
    if abs(rpt.heading_yaw_drift_rate_dps) > 2.0:
        side = "右" if rpt.heading_yaw_drift_rate_dps > 0 else "左"  # MCU yaw CW+: yaw>0=右转
...
def format_report(rpt: StraightLineReport, params: dict[str, Any], seq: int = 0) -> str:
    ...
    L.append(f"  直线 PID 深度分析报告 v8  (scoring v10 + drift v11){seq_tag}")
    L.append("=" * W)
    ...
    def _pf(k, w=5):
        v = params.get(k, '?')
        return f"{v:.{w}f}" if isinstance(v, (int, float)) else str(v)
    L.append(f"  SKP={_pf('skp')}  SKI={_pf('ski')}  SKD={_pf('skd')}")
    L.append(f"  AKP={_pf('akp')}  AKI={_pf('aki')}  AKD={_pf('akd')}")
    L.append(f"  SFF={_pf('sff',3)}  SPD={_pf('target_speed',1)}")

    # ===== MULTI-METHOD DRIFT ANALYSIS =====
    L.append(f"\n{'━' * W}")
    L.append(f"【漂移方向分析】  (5方法加权共识)")
    if rpt.drift_consensus:
        icon = '←' if '左' in rpt.drift_consensus else ('→' if '右' in rpt.drift_consensus else '·')
        L.append(f"  ★ 共识: {icon} {rpt.drift_consensus}  (置信度 {rpt.drift_confidence}%)")
        for name, m in rpt.drift_methods.items():
            label = {'enc_diff': '编码器差分', 'pwm_diff': 'PWM差分  ',
                     'hi': '航向积分  ', 'yaw_trend': 'Yaw趋势  ',
                     'odometry': '修正里程计'}.get(name, name)
            L.append(f"    {label}: [{m['dir']}] {m['desc']}")
    else:
        L.append(f"  (数据不足)")

    # ===== RAW SPEED CONTINUITY =====
    L.append(f"\n{'━' * W}")
    L.append(f"【速度连贯性】  (原始数据, 稳态段)")
    L.append(f"  OL(实际PWM): std={rpt.raw_ol_std:.1f}  slew均值={rpt.raw_ol_slew_mean:.1f}  "
             f"slew最大={rpt.raw_ol_slew_max}  突变>20: {rpt.raw_ol_slew_gt20_pct:.1f}%")
    L.append(f"  pc(PID输出): std={rpt.raw_pc_std:.1f}  振幅={rpt.raw_pc_range}  "
             f"停顿(pc<=1): {rpt.raw_pc_stall_pct:.1f}%")
    L.append(f"  el(编码器):  CV={rpt.raw_el_cv:.1f}%")
    # Traffic light
    if rpt.raw_ol_std <= 5:
        L.append(f"  → ✅ 速度稳定")
    elif rpt.raw_ol_std <= 10:
        L.append(f"  → ⚠ 速度有波动, 可感知时快时慢")
    else:
        L.append(f"  → 🔴 速度时快时慢严重!")

    # ===== SPEED RESPONSE =====
    L.append(f"\n【速度响应】  (目标={params.get('target_speed','?')})")
    L.append(f"  上升: {rpt.speed_rise_time_ms}ms  超调: {rpt.speed_overshoot_pct:.1f}%  "
             f"建立: {rpt.speed_settle_time_ms}ms")
    L.append(f"  稳态: mean={rpt.speed_ss_mean:.2f}  err={rpt.speed_ss_error:+.2f}({rpt.speed_ss_error_pct:+.1f}%)  "
             f"RMSE={rpt.speed_tracking_rmse:.2f}")
    L.append(f"  编码器: L={rpt.speed_mean_el:+.1f}±{rpt.speed_std_el:.1f}  "
             f"R={rpt.speed_mean_er:+.1f}±{rpt.speed_std_er:.1f}  "
             f"asym={rpt.speed_asymmetry_pct:.1f}%")
    ...
    if rpt.traj_total_distance > 0:
        ...
        if abs(final_mm) > 3.0:  # >3mm drift is noticeable
            direction = "偏左(+y)" if rpt.traj_final_lateral_dev > 0 else "偏右(-y)"  # y>0=左偏 (yaw negated: CW+→CCW+, sin(+)→y>0=LEFT)
    ...
    # ===== SCORE =====
    L.append(f"\n{'━' * W}")
    L.append(f"【综合评分】  {rpt.straightness_score:.1f} / 100  (v11 drift-corrected){seq_tag}")
...
def save_results(result: CaptureResult, rpt: StraightLineReport,
    ...
    with csv_path.open("w", encoding="utf-8") as f:
        f.write("t_ms,mode,run,el,er,yaw,yr,pc,hd,dp,OL,OR,hi,sb,lp\n")
        for r in result.records:
            f.write(f"{r.t_ms},{r.mode},{r.run},{r.el},{r.er},"
                    f"{r.yaw:.1f},{r.yr:.1f},{r.pc},{r.hd},{r.dp},"
                    f"{r.ol},{r.or_},{r.hi:.3f},{r.sb},{r.lp:.2f}\n")
...
def main() -> None:
    ...
    def _pp(d, k, w=5):
        v = d.get(k, '?')
        return f"{v:.{w}f}" if isinstance(v, (int, float)) else str(v)
    print(f"  PID: SKP={_pp(pid_params,'skp')} SKI={_pp(pid_params,'ski')} SKD={_pp(pid_params,'skd')}")
    print(f"       AKP={_pp(pid_params,'akp')} AKI={_pp(pid_params,'aki')} AKD={_pp(pid_params,'akd')}")
    if 'sff' in pid_params:
        print(f"  前馈: SFF={_pp(pid_params,'sff',3)}")
