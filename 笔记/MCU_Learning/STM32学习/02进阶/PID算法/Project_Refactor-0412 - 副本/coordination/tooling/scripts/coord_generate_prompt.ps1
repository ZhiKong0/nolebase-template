...
try {
  ...
  $sessionHeads = Get-ProjectSessionHeadsFile -Root $root -ProjectId $ProjectId
  ...
  $streamEntries = Get-ProjectStreamEntriesFile -Root $root -ProjectId $ProjectId
  $streamTail = Get-ProjectStreamTailFile -Root $root -ProjectId $ProjectId
  $stateEventLog = Get-ProjectStateEventLogFile -Root $root -ProjectId $ProjectId
  $transactionsRoot = Get-ProjectTransactionsPath -Root $root -ProjectId $ProjectId
...
4. Follow ``$projectAiStartFile`` exactly. Its minimal chain starts with:
   ...
   - ``$digestLatest``
   - ``$activeSessionsIndex``
5. Use unread delta polling for live collaboration:
   - ``coordination\bin\coord-read-stream.cmd -ProjectId $ProjectId -AgentId $resolvedAgentName -OtherAgentsOnly -IncludeSessionHeads``
   - Treat ``$streamNewestEntry`` and ``$streamTail`` as summary projections only.
6. Read ``$taskFile``
7. Read ``$lockFile``
8. Read ``$sessionDeltaFile``
9. Read this generated prompt: ``$promptPath``
10. Read ``$boardFile`` and ``$handoffFile`` if queue state or baton history matters
11. Read ``$decisionsFile`` if design decisions matter
12. Read ``$readmeIndex`` only if you need deeper topology or boundaries
13. Read ``$architectureTree`` and ``$architectureIndex`` only if deeper system structure matters
14. Read ``$recentIndex`` only if the task overlaps recent work
15. Read ``$baselineState`` only if baseline ``$baselineVersion`` is new to you
...
- During multi-AI work, poll the shared tail with:
  ``coordination\bin\coord-read-stream.cmd -ProjectId $ProjectId -AgentId $resolvedAgentName -OtherAgentsOnly -IncludeSessionHeads``
- Treat ``$streamEntries`` as the live source of truth for the active runtime.
- Treat ``$streamNewestEntry`` and ``$streamTail`` as atomic summary projections.
- If `coord-read-stream` reports a missing pointer target, stop at the lossy boundary, reread ``$liveStateBoard`` and ``$digestLatest``, then rerun with ``-AcceptLossyBoundary`` only when you intentionally accept resuming from the current stream head.
...
- Session heads: ``$sessionHeads``
- Digest head: ``$digestLatest``
- Stream source: ``$streamEntries``
- Stream newest summary: ``$streamNewestEntry``
- Stream tail summary: ``$streamTail``
- State event log: ``$stateEventLog``
...
"@
  ...
  if ($DryRun) {
    Write-JsonResult @{
      ...
      streamEntriesFile = $streamEntries
      streamNewestEntryFile = $streamNewestEntry
      streamTailFile = $streamTail
      branch = $resolvedBranch
  ...
  Invoke-WithProjectLock -Root $root -ProjectId $ProjectId -LockName "runtime" -ScriptBlock {
    param($lockPath)
    ...
    if (Test-Path -LiteralPath $sessionDeltaFile) {
      ...
      $sessionDelta["paths"]["streamEntries"] = $streamEntries
      $sessionDelta["paths"]["streamNewestEntry"] = $streamNewestEntry
      $sessionDelta["paths"]["streamTail"] = $streamTail
      $sessionDelta["paths"]["stateEventLog"] = $stateEventLog
      $sessionDelta["paths"]["transactionsRoot"] = $transactionsRoot
      $sessionDelta["updatedAt"] = Get-NowIso
      Write-JsonFile -Path $sessionDeltaFile -Object $sessionDelta
    }

    if ($null -ne $sessionEntry) {
      $currentRegistry = Read-ProjectRegistry -Root $root -ProjectId $ProjectId
      $currentRegistry["sessions"] = Upsert-ListEntry -Items $currentRegistry["sessions"] -Item @{
        sessionId = $resolvedSessionId
        taskId = $TaskId
      ...
      Write-ProjectRegistry -Root $root -ProjectId $ProjectId -Registry $currentRegistry
    }
  }
