...
try {
  ...
  $result = Invoke-WithProjectLock -Root $root -ProjectId $ProjectId -LockName "runtime" -ScriptBlock {
    $sessionDeltaLocked = Read-JsonFile -Path $sessionDeltaPath
    $registryLocked = Read-ProjectRegistry -Root $root -ProjectId $ProjectId
    $conflictLock = $null
    if ($writeLikeActions -contains $normalizedAction) {
      foreach ($lockEntry in ($registryLocked["locks"] | ForEach-Object { $_ })) {
    ...
    if ($null -ne $conflictLock) {
      ...
      if (-not $sessionDeltaLocked.ContainsKey("delta") -or -not ($sessionDeltaLocked["delta"] -is [hashtable])) {
      ...
      $sessionDeltaLocked["delta"]["conflicts"] = @($sessionDeltaLocked["delta"]["conflicts"] | ForEach-Object { $_ }) + @(@{
        targetPath = $targetRelativePath
        conflictingTaskId = $conflictLock["taskId"]
        conflictingSessionId = $conflictLock["sessionId"]
        loggedAt = $conflictRecord["timestamp"]
      })
      $sessionDeltaLocked["updatedAt"] = $conflictRecord["timestamp"]
      $sessionDeltaLocked["lastOperation"] = $conflictRecord
      Write-JsonFile -Path $sessionDeltaPath -Object $sessionDeltaLocked
    ...
    if (-not $sessionDeltaLocked.ContainsKey("counters") -or -not ($sessionDeltaLocked["counters"] -is [hashtable])) {
      $sessionDeltaLocked["counters"] = @{
        read = 0
        write = 0
        modify = 0
        delete = 0
    ...
    $counterKey = if ($sessionDeltaLocked["counters"].ContainsKey($normalizedAction)) { $normalizedAction } else { "other" }
    $sessionDeltaLocked["counters"][$counterKey] = [int]$sessionDeltaLocked["counters"][$counterKey] + 1
    $sessionDeltaLocked["updatedAt"] = $record["timestamp"]
    $sessionDeltaLocked["lastOperation"] = $record
    $sessionDeltaLocked["delta"]["summary"] = $DeltaSummary

    $touchedFiles = @($sessionDeltaLocked["delta"]["touchedFiles"] | ForEach-Object { $_ })
    $pendingWrites = @($sessionDeltaLocked["delta"]["pendingWrites"] | ForEach-Object { $_ })
    ...
    if ((@("write", "modify", "delete", "patch") -contains $normalizedAction) -and ($pendingWrites -notcontains $targetRelativePath)) {
      ...
      $sessionDeltaLocked["delta"]["touchedFiles"] = @($touchedFiles)
      $sessionDeltaLocked["delta"]["pendingWrites"] = @($pendingWrites)
    Write-JsonFile -Path $sessionDeltaPath -Object $sessionDeltaLocked
    ...
    if (Test-Path -LiteralPath $activeIndexFile) {
      ...
      if ($activeIndex.ContainsKey("sessions")) {
        $activeIndex["sessions"] = Upsert-ListEntry -Items $activeIndex["sessions"] -Item @{
          ...
          taskId = $TaskId
          agentId = $AgentId
          status = if ($sessionDeltaLocked.ContainsKey("status")) { [string]$sessionDeltaLocked["status"] } else { "active" }
