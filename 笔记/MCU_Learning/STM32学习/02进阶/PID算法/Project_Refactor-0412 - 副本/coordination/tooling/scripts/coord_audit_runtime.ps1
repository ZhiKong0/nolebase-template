...
try {
  ...
  foreach ($currentProjectId in $projectIds) {
    ...
    $requiredProjectFiles = @(
      ...
      (Get-ProjectStreamEntriesFile -Root $root -ProjectId $currentProjectId),
      (Get-ProjectStreamNewestEntryFile -Root $root -ProjectId $currentProjectId),
      (Get-ProjectStreamTailFile -Root $root -ProjectId $currentProjectId),
      (Get-ProjectSessionHeadsFile -Root $root -ProjectId $currentProjectId),
      (Get-ProjectDigestLatestSummaryFile -Root $root -ProjectId $currentProjectId),
      (Get-ProjectStateEventLogFile -Root $root -ProjectId $currentProjectId),
    ...
    foreach ($dirPath in @(
        (Get-ProjectActiveTransactionsPath -Root $root -ProjectId $currentProjectId),
        (Get-ProjectTransactionHistoryPath -Root $root -ProjectId $currentProjectId),
        (Get-ProjectInternalLocksPath -Root $root -ProjectId $currentProjectId)
      )) {
      if (-not (Test-Path -LiteralPath $dirPath)) {
        Add-Finding -Collection ([ref]$issues) -Finding (New-Finding -Level "error" -Code "missing_project_dir" -Message ("Missing project directory: {0}" -f $dirPath) -ProjectId $currentProjectId -Path $dirPath)
      }
    }
    ...
    if (Test-Path -LiteralPath $aiStartFile) {
      ...
      if (-not (Test-TokenOrder -Text $aiStartText -Earlier "STREAM_LOG/SESSION_HEADS.json" -Later "README_INDEX.md")) {
        Add-Finding -Collection ([ref]$warnings) -Finding (New-Finding -Level "warning" -Code "ai_start_session_heads_order" -Message "AI_START.md does not place SESSION_HEADS before README_INDEX." -ProjectId $currentProjectId -Path $aiStartFile)
      }
      ...
      if (-not $aiStartText.Contains("coord-read-stream.cmd")) {
        Add-Finding -Collection ([ref]$warnings) -Finding (New-Finding -Level "warning" -Code "ai_start_missing_stream_command" -Message "AI_START.md does not mention coord-read-stream for unread live deltas." -ProjectId $currentProjectId -Path $aiStartFile)
      }
    }
    ...
    if (Test-Path -LiteralPath $readmeIndexFile) {
      ...
      if (-not $readmeIndexText.Contains('Prefer `coord-read-stream`')) {
        Add-Finding -Collection ([ref]$warnings) -Finding (New-Finding -Level "warning" -Code "readme_index_missing_stream_rule" -Message "README_INDEX.md does not prefer coord-read-stream for unread live deltas." -ProjectId $currentProjectId -Path $readmeIndexFile)
      }
    }
    ...
    foreach ($session in $activeSessions) {
      ...
      $thoughtLog = Get-ProjectSessionTriadFile -Root $root -ProjectId $currentProjectId -SessionId $sessionId -State "active" -TriadType "THOUGHT"
      $execLog = Get-ProjectSessionTriadFile -Root $root -ProjectId $currentProjectId -SessionId $sessionId -State "active" -TriadType "EXEC"
      $modLog = Get-ProjectSessionTriadFile -Root $root -ProjectId $currentProjectId -SessionId $sessionId -State "active" -TriadType "MOD"
