...
| `state/events/STATE_EVENTS.jsonl` | append-only state transition log | `coord-init-project`, `coord-claim-task`, `coord-context-digest` |
| `state/transactions/` | transaction metadata for claim/init/digest flows | `coord-init-project`, `coord-claim-task`, `coord-context-digest` |
```
...
```json
{
  ...
  "paths": {
    ...
    "streamEntries": "D:\\repo\\coordination\\runtime\\projects\\my-app\\STREAM_LOG\\STREAM.jsonl",
    ...
    "stateEventLog": "D:\\repo\\coordination\\runtime\\projects\\my-app\\state\\events\\STATE_EVENTS.jsonl",
    "transactionsRoot": "D:\\repo\\coordination\\runtime\\projects\\my-app\\state\\transactions"
  },
  "triadCounters": {
    "thought": 0,
    "exec": 0,
...
4. Incremental read:
   If baseline already loaded and unchanged, read `LIVE_STATE_BOARD.md`, `STREAM_LOG/SESSION_HEADS.json`, `L3_DIGEST/latest_summary.md`, `L1_ACTIVE`, then use `coord-read-stream` for unread live deltas. Treat `STREAM_LOG/NEWEST_ENTRY.json` and `STREAM_LOG/LATEST_TAIL.json` as summary projections only. Read `README_INDEX.md` only when the task needs deeper topology or boundary detail.
5. Before action:
...
- Pointer gap handling:
  If the per-agent pointer target no longer exists in the current stream source, stop at the lossy boundary. Read `LIVE_STATE_BOARD.md`, `SESSION_HEADS.json`, and `L3_DIGEST/latest_summary.md` first, then rerun `coord-read-stream` with `-AcceptLossyBoundary` only when you intentionally accept resuming from the current stream head.
