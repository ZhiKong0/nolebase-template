...
- After the digest head is known, later task resumes should read `LIVE_STATE_BOARD.md`, `SESSION_HEADS.json`, `L1_ACTIVE`, and your `SESSION_DELTA.json` first.
- Prefer `coord-read-stream` for unread live deltas; treat `NEWEST_ENTRY.json` and `LATEST_TAIL.json` as summaries, not source-of-truth history.
...
5. `coord-read-stream` with your current mode-specific pointer
6. your own `SESSIONS/active/<session-id>/SESSION_DELTA.json`
7. your task file
8. your lock file
