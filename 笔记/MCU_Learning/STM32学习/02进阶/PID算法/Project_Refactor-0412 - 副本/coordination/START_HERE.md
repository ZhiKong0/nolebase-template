...
6. Require every AI to start each repository-scoped turn by checking `coordination\AI_ENTRY.md` or running `coordination\bin\coord-detect-context.cmd`.
7. Require every AI to log `THOUGHT`, `EXEC`, and `MOD` with `coord-log-thought`, `coord-log-operation`, and `coord-log-mod`.
8. Tell active AIs to use `coordination\bin\coord-read-stream.cmd -ProjectId <id> -AgentId <agent> -OtherAgentsOnly -IncludeSessionHeads` for low-latency context polling instead of loading raw history.
9. If the baton changes while the task stays active, run `coordination\bin\coord-handoff-task.cmd`.
10. If a closed task needs to resume later, run `coordination\bin\coord-reopen-task.cmd`.
11. When the task pauses or ends, make sure the worktree is committed or clean, then run `coordination\bin\coord-close-task.cmd`; this also appends the `L3_DIGEST` summary node.
12. When the repository no longer needs active coordination runtime, run `coordination\bin\coord-archive-project.cmd`.
