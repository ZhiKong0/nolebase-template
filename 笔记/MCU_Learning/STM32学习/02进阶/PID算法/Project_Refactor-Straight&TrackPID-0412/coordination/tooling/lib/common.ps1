...
function Update-LiveStateBoard {
  ...
  if (@($blockerLines).Count -eq 0) {
    $blockerLines = @("- No active blockers recorded in the latest stream tail.")
  }
  ...
  if (@($tailLines).Count -eq 0) {
    $tailLines = @("- Stream tail is empty.")
  }
...
function Write-TriadRecord {
  ...
  $tailState = if (Test-Path -LiteralPath $tailFile) { Read-JsonFile -Path $tailFile } else { @{ version = 1; updatedAt = $timestamp; maxEntries = Get-StreamTailSize; entries = @() } }
  if (-not $tailState.ContainsKey("entries")) {
    $tailState["entries"] = @()
  }
  $tailEntries = ConvertTo-ObjectArray -Value $tailState["entries"]
  $tailEntries += ,$payload
  $maxEntries = if ($tailState.ContainsKey("maxEntries")) { [int]$tailState["maxEntries"] } else { Get-StreamTailSize }
  $tailEntries = @($tailEntries | Sort-Object {
      if ($_.ContainsKey("timestamp")) { [DateTimeOffset]::Parse([string]($_["timestamp"])) } else { [DateTimeOffset]::MinValue }
    } | Select-Object -Last $maxEntries)
  $tailState["version"] = 1
  $tailState["updatedAt"] = $timestamp
  $tailState["maxEntries"] = $maxEntries
  $tailState["entries"] = $tailEntries
  Write-JsonFile -Path $tailFile -Object $tailState

  Write-JsonFile -Path $newestEntryFile -Object @{
    version = 1
    updatedAt = $timestamp
    entry_id = $entryId
    project_id = $ProjectId
    ...
    type_name = $typeName
    status = $Status
    triad_file = $triadFile
    content_preview = (Get-TextPreview -Text $Content -MaxLength 160)
    related_files = $normalizedFiles
  }
...
function Update-ProjectAiStartFile {
  ...
  $content = @(
    ...
    ('- Newest stream entry: `{0}`' -f $streamNewestEntryFile)
    ('- Board: `{0}`' -f $boardFile)
    ...
    ('- STREAM: `coordination\bin\coord-read-stream.cmd -ProjectId {0} -AgentId <agent-id>`' -f $ProjectId)
    ""
    "## Rule"
    ""
...
function Update-CoordinationActiveContext {
  ...
  foreach ($record in $projectRecords) {
    $line = '- Project `{0}` repo=`{1}` active_sessions={2} ai_start=`{3}` live_board=`{4}` digest=`{5}`' -f $record["projectId"], $record["repoPath"], $record["activeSessionCount"], $record["aiStartFile"], $record["liveStateBoardFile"], $record["digestLatestFile"]
