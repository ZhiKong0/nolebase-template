/*
 * 传感器融合与底层采样:
 * 1. 前半部分是 BNO085 软件 I2C + SHTP 协议实现。
 * 2. 后半部分是 8 路循迹传感器 GPIO 采样和位置加权。
 * 3. 这里的职责是“把原始传感器信号整理成稳定观测量”，不直接参与控制决策。
 */
...
/* 以上静态变量组成 BNO085 初始化/收包状态机，集中放在文件内避免外部误改。 */

static void bno_diag_update(void)
{
    /* IMU 初始化状态会投到 OLED 上，方便上电时区分“没连上”和“还在初始化”。 */
...
static void bno_try_recover(IMU_Data_t *data)
{
    ...
    /* 运行中如果在这里直接重跑 BNO085_Init，会把主循环卡住上百到上千毫秒，
       电机只能沿用上一拍 PWM，表现出来就是“屏上 IMU 退回 1，车开始自己拧”。
       这里改成只把姿态判成无效，由主循环在停车态再调用 BNO085_Service 重连。 */
}
...
{
    /* 统计当前有多少路检测到线，用于判断是否有效以及做位置加权平均。 */
    uint8_t c = 0, i;
    for (i = 0; i < 8; i++) {
        if (bits & (1u << i)) c++;
    }
