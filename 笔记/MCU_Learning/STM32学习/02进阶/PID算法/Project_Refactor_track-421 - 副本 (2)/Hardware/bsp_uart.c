                               ...
                               int16_t pwmL, int16_t pwmR)
{
    ...
    sprintf(buf,
        "HB:t=%lu,m=P,run=%u,exp=%lu,el=%d,er=%d,yaw=%.1f,yr=%.1f,"
        "OL=%d,OR=%d\r\n",
        (unsigned long)tMs, (unsigned)run, (unsigned long)experimentId,
        (int)encL, (int)encR,
        (double)yaw, (double)yawRate,
                                ...
                                uint8_t dbgCornerAcceptHit)
{
    ...
    if (dbgTrackState == 1u) stateStr = "TURN";
    ...
    sprintf(buf,
        ...
        "pb=%u,ga=%.2f,lkp=%.2f,lkd=%.2f,lca=%.2f,ltr=%.2f,cts=%.2f,"
        ...
        (unsigned)pidBypassActive,
        (double)gainAlpha, (double)activeLkp, (double)activeLkd,
        (double)activeCenterAnchor, (double)activeSteerTrim, (double)curveTargetSpeed,
                      ...
                      float curveSpeedMinRatio)
{
    ...
    if (mode == MODE_TRACK) modeStr = "TRACK";
    else if (mode == MODE_SPIN) modeStr = "SPIN";
    ...
    sprintf(buf,
        ...
        "tdyn=%u,tkp0=%.4f,tkp1=%.4f,tkd0=%.4f,tkd1=%.4f,"
        "tdb0=%.4f,tdb1=%.4f,tcl0=%.4f,tcl1=%.4f,"
        "tca0=%.4f,tca1=%.4f,ttr=%.4f,tbg=%.4f,tsmr=%.4f\r\n",
        stateStr,
        modeStr,
        (double)targetSpeed,
        ...
        (unsigned)dynamicPidEnabled,
        (double)kpStraight, (double)kpCurve,
        (double)kdStraight, (double)kdCurve,
        (double)deadbandStraight, (double)deadbandCurve,
        (double)loadLow, (double)loadHigh,
        (double)centerAnchorStraight, (double)centerAnchorCurve,
        (double)steerTrim,
        (double)curveBrakeGain,
        (double)curveSpeedMinRatio);
    BspUart_SendString(buf);
}
