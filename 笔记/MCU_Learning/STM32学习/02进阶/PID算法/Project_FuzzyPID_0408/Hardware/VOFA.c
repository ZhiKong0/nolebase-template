...
static volatile uint8_t rxFrameType = 0u;
...
uint8_t VOFA_TakeCommand(char *out, uint16_t outSize) {
    uint16_t i;
    uint16_t cmdLen;
    uint32_t primask;
    ...
    if (((rxFrameType == 1u) && ((rxIndex < 2u) || (rxBuffer[0] != '#') || (rxBuffer[rxIndex - 1u] != '!'))) ||
        ((rxFrameType == 2u) && ((rxIndex < 2u) || (rxBuffer[0] != '{')))) {
        rxState = VOFA_STATE_WAIT_HEAD;
        rxIndex = 0;
        rxFrameType = 0u;
        out[0] = '\0';
    ...
    if (rxFrameType == 1u) {
        cmdLen = (uint16_t)(rxIndex - 1u);
    } else {
        cmdLen = rxIndex;
    }
    if (cmdLen >= outSize) {
        cmdLen = (uint16_t)(outSize - 1u);
    }
    for (i = 0u; i < cmdLen; i++) {
        out[i] = rxBuffer[i];
    }
