                                         ...
                                         IMU_Data_t *tempData)
{
    ...
    if (!bno_wait_present(presentTimeoutMs)) { s_bnoAddr = 0; return 0u; }
    if (!bno_wait_present_stable(
            (uint16_t)(BNO_PRESENT_STABLE_CNT * BNO_PRESENT_STABLE_GAP * 6u),
            BNO_PRESENT_STABLE_CNT)) {
        s_bnoAddr = 0; return 0u;
    }

    s_bnoInitStage = BNO_INIT_PID;
    (void)bno_request_product_id();

    s_bnoInitStage = BNO_INIT_FEATURE;
    ...
    /* Enable calibrated gyroscope for direct angular velocity (D-term).
       ...
       and zero differentiation noise.
       Use 10ms interval (100Hz) — different from GAME_ROT_VEC (20ms)
       to prevent BNO085 from batching them into one packet, which
       doubles I2C read time and halves the control loop rate. */
    (void)bno_enable_report(BNO_SENSOR_GYRO, 40000UL);

    s_bnoReady = 1u;
    s_bnoInitStage = BNO_INIT_WAIT;
    if (!bno_wait_first_report(tempData, BNO_FIRST_REPORT_MS)) {
        s_bnoReady = 0u; s_bnoWho = 0u; s_bnoAddr = 0; return 0u;
    }

    s_bnoWho = 0x85u;
    s_bnoInitStage = BNO_INIT_READY;
    return 1u;
}
