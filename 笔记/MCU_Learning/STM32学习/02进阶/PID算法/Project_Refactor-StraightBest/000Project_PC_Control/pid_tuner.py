...
def reconstruct_trajectory(records: list[HBRecord]) -> list[TrajectoryPoint]:
    ...
    for r in records:
        ...
        heading_rad = math.radians(r.yaw)
        x += distance * math.cos(heading_rad)
        y += distance * math.sin(heading_rad)
...
def analyze_straight(records: list[HBRecord], params: dict[str, Any],
    ...
    if traj and len(traj) > 2:
        ...
        if fr_speeds:
            rpt.fr_speed_mean = round(statistics.mean(fr_speeds), 1)
            rpt.fr_speed_std = round(statistics.stdev(fr_speeds) if len(fr_speeds) > 1 else 0, 1)
            rpt.fr_speed_min = round(min(fr_speeds), 1)
            rpt.fr_speed_max = round(max(fr_speeds), 1)
        ...
        if len(cruise_speeds) > 2:
            rpt.fr_cruise_speed_mean = round(statistics.mean(cruise_speeds), 1)
            rpt.fr_cruise_speed_std = round(statistics.stdev(cruise_speeds), 1)
            rpt.fr_cruise_speed_cv = round(
                rpt.fr_cruise_speed_std / max(rpt.fr_cruise_speed_mean, 0.1) * 100, 1)
        ...
        # --- Lateral tracking in mm ---
        lat_mms = [p.lateral_dev * ENC_TO_MM for p in traj]
        rpt.fr_lat_max_mm = round(max(abs(l) for l in lat_mms), 1)
        rpt.fr_lat_final_mm = round(lat_mms[-1], 1)
        lat_ts = [p.t_ms / 1000.0 for p in traj]
        rpt.fr_lat_drift_rate_mm_s = round(_linfit(lat_ts, lat_mms), 2)
        ...
        for i, p in enumerate(traj):
            ...
            if t_s >= next_display:
                ...
                rpt.fr_timeline.append({
                    't': round(t_s, 1),
                    'fwd': round(p.x * ENC_TO_MM, 0),
                    'lat': round(p.lateral_dev * ENC_TO_MM, 1),
                    'lat_v': round(lat_spd, 1),
                    'spd': round(spd, 0),
                    'yaw': round(p.yaw_deg, 2),
                    'yr': round(rec.yr, 1),
                    'pc': rec.pc,
                    'hd': rec.hd,
                    'dp': rec.dp,
    ...
    se_pen = min(pc_err_pct * 0.2, 12)
    sstd_pen = min(rpt.speed_pc_ss_std * 0.8, 6) if sr else min(rpt.speed_std_pc * 0.6, 6)
    sov_pen = min(rpt.speed_overshoot_pct * 0.03, 6)
    srise_pen = min(max(rpt.speed_rise_time_ms - 600, 0) / 400, 5)
    scv_pen = min(max(rpt.fr_cruise_speed_cv - 5, 0) * 0.1, 3)  # CV>5% starts penalty
    sosc_pen = min(rpt.fr_pc_osc_amplitude * 0.3, 2) if rpt.fr_pc_osc_detected else 0
    rpt.score_speed = round(max(0, 25 - se_pen - sstd_pen - sov_pen - srise_pen - scv_pen - sosc_pen), 1)

    # Heading (30 pts): uses post-startup metrics (startup transient ≠ steady-state quality)
    hdr_pen = min(abs(rpt.heading_yaw_drift_rate_dps) * 4, 12)
    ...
    hmax_pen = min(_ss_max * 0.4, 10)
    hstd_pen = min(_ss_std * 0.4, 8)
    hyint_pen = min(_ss_int * 0.04, 5)  # accumulated yaw error
    ...
    rpt.score_heading = round(max(0, 30 - hdr_pen - hmax_pen - hstd_pen - hyint_pen - hsat_pen - hsign_pen), 1)

    # Trajectory (25 pts): mm-based lateral deviation + drift rate
    fwd_mm = rpt.traj_total_distance * ENC_TO_MM  # forward distance in mm
    fwd_m = max(fwd_mm / 1000.0, 0.01)
    # Use mm-based metrics from full-rate analysis
    tlat_pen = min(rpt.fr_lat_max_mm / fwd_m * 0.3, 12) if traj else 6  # mm/m penalty
    tlatm_pen = min(abs(rpt.fr_lat_final_mm) / fwd_m * 0.6, 8) if traj else 3
    tlatv_pen = min(abs(rpt.fr_lat_drift_rate_mm_s) * 0.8, 5) if traj else 2
    tsin_pen = min(max(rpt.traj_sinuosity - 1.0, 0) * 120, 5) if traj else 3
    tasym_pen = min(rpt.speed_asymmetry_pct * 0.4, 5)
    rpt.score_trajectory = round(max(0, 25 - tlat_pen - tlatm_pen - tlatv_pen - tsin_pen - tasym_pen), 1)

    # Smoothness (20 pts): jitter, slew, reversal, idle
    smj_pen = min(rpt.jitter_score * 0.05, 10)
    smslew_pen = min(rpt.mean_pc_slew * 0.15, 6)
    smrev_pen = min(rpt.el_negative_pct * 0.6, 5)
    smidle_pen = min(max(rpt.state_idle_pct - 3, 0) * 0.2, 4)
    rpt.score_smoothness = round(max(0, 20 - smj_pen - smslew_pen - smrev_pen - smidle_pen), 1)
...
def format_report(rpt: StraightLineReport, params: dict[str, Any], seq: int = 0) -> str:
    ...
    L.append(f"  直线 PID 深度分析报告 v5  (full-rate + strict scoring){seq_tag}")
    L.append("=" * W)
    ...
    # ===== HEADING =====
    L.append(f"\n【航向控制】")
    L.append(f"  yaw: final={rpt.heading_yaw_final:+.2f}°  drift={rpt.heading_yaw_drift_rate_dps:+.3f}°/s  "
             f"std={rpt.heading_yaw_std:.2f}°  max={rpt.heading_yaw_max_abs:.2f}°")
    ...
    L.append(f"  起步yaw峰值: {rpt.startup_yaw_peak:.2f}°  "
             f"稳态drift: {rpt.steady_yaw_drift_rate:+.3f}°/s")
    L.append(f"  yaw积分: {rpt.heading_yaw_integral:.2f}°·s  "
             f"过零: {rpt.heading_yaw_zero_crossings}次  "
             f"单调: {rpt.heading_yaw_monotonic_pct:.0f}%")
    ...
    L.append(f"\n【逐帧速度分析】  (全帧率, N={rpt.run_samples}, Δt≈{rpt.sample_period_ms:.0f}ms)")
    L.append(f"  瞬时速度(mm/s): mean={rpt.fr_speed_mean:.0f}  std={rpt.fr_speed_std:.0f}  "
             f"min={rpt.fr_speed_min:.0f}  max={rpt.fr_speed_max:.0f}")
    if rpt.fr_cruise_speed_mean > 0:
        L.append(f"  巡航段(>{rpt.startup_duration_ms}ms): "
                 f"mean={rpt.fr_cruise_speed_mean:.0f}±{rpt.fr_cruise_speed_std:.0f} mm/s  "
                 f"CV={rpt.fr_cruise_speed_cv:.1f}%")
    ...
    L.append(f"\n【逐帧轨迹追踪】  (1enc≈{ENC_TO_MM}mm)")
    L.append(f"  横偏(mm): max={rpt.fr_lat_max_mm:.1f}  "
             f"终点={rpt.fr_lat_final_mm:+.1f}  "
             f"漂移率={rpt.fr_lat_drift_rate_mm_s:+.2f} mm/s")
    if rpt.fr_timeline:
        hdr = (f"  {'t':>5s} | {'fwd_mm':>7s} | {'lat_mm':>7s} | {'横偏速':>6s} | "
               f"{'spd':>5s} | {'yaw°':>6s} | {'yr':>5s} | "
        ...
        for e in rpt.fr_timeline:
            L.append(f"  {e['t']:4.1f}s | {e['fwd']:>7.0f} | {e['lat']:>+7.1f} | "
                     f"{e['lat_v']:>+6.1f} | "
                     f"{e['spd']:>5.0f} | {e['yaw']:>+6.2f} | {e['yr']:>+5.1f} | "
    ...
    # ===== SCORE =====
    L.append(f"\n{'━' * W}")
    L.append(f"【综合评分】  {rpt.straightness_score:.1f} / 100  (strict v6){seq_tag}")
    L.append(f"  速度({rpt.score_speed:.1f}/25)  航向({rpt.score_heading:.1f}/30)  "
             f"轨迹({rpt.score_trajectory:.1f}/25)  平滑({rpt.score_smoothness:.1f}/20)")
