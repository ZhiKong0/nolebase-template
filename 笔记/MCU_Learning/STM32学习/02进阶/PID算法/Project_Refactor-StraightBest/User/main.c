...
static void handle_command(const char *cmd)
{
    ...
    if (strncmp(cmd, "#LKI=", 5) == 0)
    {
        if (cmd_parse_float(cmd + 5, &fval))
        {
            g_pid.linePID.ki = fval;
            BspUart_SendString("OK:LKI\r\n");
        }
        return;
    }
    ...
    if (strcmp(cmd, "#STAT!") == 0)
    {
        BspUart_SendStat(g_sysState, g_mode,
                         ...
                         g_pid.linePID.kp, g_pid.linePID.ki, g_pid.linePID.kd,
                         g_pid.targetSpeed);
        return;
    }
    if (strcmp(cmd, "#CAL!") == 0)
...
static void run_control(uint32_t now)
{
    ...
    {
        float predictedYaw;
        float imuAge = (float)(now - g_lastImuTick) * 0.001f;
        avgSpeed = (g_encoder.filteredLeftSpeed + g_encoder.filteredRightSpeed) * 0.5f;
        predictedYaw = g_imu.yaw + g_imu.yawRate * imuAge;
        while (predictedYaw > 180.0f)
            predictedYaw -= 360.0f;
        while (predictedYaw < -180.0f)
            predictedYaw += 360.0f;
        /* D-term: use BNO085 calibrated gyroscope (gyroZf) directly.
           Gyro measures angular velocity with ~5ms latency and zero
           differentiation noise — far superior to differentiating the
           rotation vector yaw (which had 19°/s noise std).
           Light LPF α=0.5 for sensor smoothing only. */
        /* Negate gyroZf: BNO085 sensor Z-axis is inverted relative to
           the game rotation vector yaw convention (sensor mounted with
           Z pointing down).  d(yaw)/dt > 0 = left turn, but gyroZf < 0
           for left turn.  Negation aligns the sign conventions. */
        g_fastYawRate += 0.5f * (-g_imu.gyroZf - g_fastYawRate);
        DualLoop_ComputeStraight(&g_pid, avgSpeed, predictedYaw, g_imu.yawRate, g_fastYawRate, dt);
    }
    else
    {
    ...
    MotorDriver_SetDiffPWM(g_pid.leftPWM, g_pid.rightPWM, 0);
}
...
int main(void)
{
    ...
    while (1)
    {
        ...
        /* 2b. IMU update (low rate, decoupled from PID) */
